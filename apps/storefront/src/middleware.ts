...
async function getRegionMap(cacheId: string) {
  ...
  if (!BACKEND_URL) {
    console.warn(
      "Middleware.ts: NEXT_PUBLIC_MEDUSA_BACKEND_URL environment variable is not defined."
    )
    return regionMap
  }
  ...
  if (
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`Middleware: Backend returned status ${response.status}`)
        return regionMap
      }
      ...
      console.error("Middleware: Error fetching regions from Medusa backend:", error)
      return regionMap
    }
  }
