...
const ImageGallery = ({ images, selectedVariant, selectedColor }: ImageGalleryProps) => {
  ...
  if (!displayImages.length) {
    return (
      <div className="w-full aspect-[3/4] bg-gray-100 rounded-2xl flex items-center justify-center text-gray-400 text-sm font-medium">
  ...
  return (
    <div className="flex flex-col-reverse sm:flex-row gap-3.5 sm:gap-4 items-start w-full">
      {/* ── Vertical Thumbnails (Left side on Desktop, Bottom on Mobile) ── */}
      {displayImages.length > 1 && (
        <div className="flex sm:flex-col gap-2.5 shrink-0 w-full sm:w-20 overflow-x-auto sm:overflow-y-auto sm:max-h-[620px] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {displayImages.map((image, index) => {
            ...
            return (
              <button
                ...
                type="button"
                className={`relative w-16 sm:w-20 shrink-0 aspect-[3/4] rounded-xl overflow-hidden border-2 transition-all duration-200 cursor-pointer bg-gray-50 ${
                  isActive
                    ? "border-brand-teal shadow-xs scale-[1.02]"
      ...
      <div className="relative flex-1 aspect-[3/4] w-full bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 shadow-2xs group flex items-center justify-center">
        {currentImage?.url && (
          <Image
            ...
            className="object-contain object-center transition-transform duration-500 group-hover:scale-105 p-1.5"
          />
        )}
        ...
        {displayImages.length > 1 && (
          <>
            ...
            {/* Counter Badge */}
            <div className="absolute bottom-3 right-3 bg-black/70 backdrop-blur-xs text-white text-[11px] px-2.5 py-1 rounded-full font-bold tracking-wider">
